...
--vypise autory zadane knihy (spojeni tri tabulek)
select AUTOR.MENO, AUTOR.PRIEZVISKO from AUTOR
inner join AUTOR_TITUL on AUTOR.ID = AUTOR_TITUL.ID_AUTOR
inner join TITUL on AUTOR_TITUL.ID_TITUL = TITUL.ID
where TITUL.NAZEV = 'Jak spravne na projekt do predmetu IFJ';

--vypise dostupne vytisky knih i s nazvem one knihy (spojeni dvou tabulek)
select VYTISK.DOSTUPNOST, VYTISK.ID, TITUL.NAZEV from VYTISK
inner join TITUL on VYTISK.ID_TITUL = TITUL.ID
where VYTISK.DOSTUPNOST = 'A';

--vypise ctenare s nevracenymi pujckami (spojeni dvou tabulek)
select CITATEL.MENO, CITATEL.PRIEZVISKO, POZICKA.STAV from CITATEL
inner join POZICKA on CITATEL.ID = POZICKA.ID_CITATELA
where POZICKA.STAV = 'Nevratene';

--vypise prijmeni ctenaru, seskupi je podle abecedy a vypise pocty vyskytu tohoto prijmeni(GROUP BY, COUNT)
select PRIEZVISKO, COUNT(*) as VYSKYTY 
from CITATEL group by PRIEZVISKO;

--vypise, spocita a spoji roky vydani vsech knih v databazi a seradi je od nejstarsi (GROUP BY, COUNT)
select ROK_VYDANIA, COUNT(*) from TITUL
group by ROK_VYDANIA order by ROK_VYDANIA;

--zobrazi loginy ctenaru, kteri si uz pres system neco rezervovali (predikat EXISTS)
select CITATEL.LOGIN from CITATEL
where exists (
select REZERVACIA.ID_CITATELA from REZERVACIA 
where CITATEL.ID = REZERVACIA.ID_CITATELA
);

--vybere a zobrazi ty autory, jenz pisi ve specifikovanem zanru (predikat IN)
select MENO, PRIEZVISKO from AUTOR
where ZANER in ('apokalypsa','SCI-FI');