...
BIG_DICT = join(THIS_DIR, "big.txt")
BAD_WORDS = join(THIS_DIR, "bad.txt")
GOOD_WORDS = join(THIS_DIR, "good.txt")
SAVED_MODEL = join(THIS_DIR, "gib_model.pki")
...
def train():
    ...
    with open(BIG_DICT, mode='r') as f:
    ...
    # bad phrases.
    good_probs = [avg_transition_prob(l, counts) for l in open(GOOD_WORDS)]
    bad_probs = [avg_transition_prob(l, counts) for l in open(BAD_WORDS)]
